...
# just testing to understand weight extraction from MHA linear layers
...
class MultiheadAttentionSuper(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, incremental_state=None,
        ...
        #q *= self.scaling # https://github.com/pytorch/xla/issues/2369
        q = q * self.scaling
...
def test():
    ...
    is_encoder = True
    qkv_dim = 768
    mha = MultiheadAttentionSuper(super_embed_dim, num_heads, is_encoder, super_kdim=None, super_vdim=None, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, self_attention=True, encoder_decoder_attention=False, out_dim=None, qkv_dim=None)
    mha.set_sample_config(sample_q_embed_dim=768, sample_attention_heads=12)
    x = torch.Tensor(10,4,768)
    print(x.size())
    x, _ = mha(query=x, key=x, value=x, key_padding_mask=None)
    print(x.size())
