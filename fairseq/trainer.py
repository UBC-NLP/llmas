...
class Trainer(object):
    ...
    def set_sample_config(self, config, arch_embeds=None):
        self._model.set_sample_config(config, arch_embeds=arch_embeds)
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False, configs=[None], args=None):
        ...
        subnet_grads = torch.zeros(num_params)
        largest_net_grads = torch.zeros(num_params)

        for ci, config in enumerate(configs):
            if config is not None:
                self.set_sample_config(config, arch_embeds=utils.get_config_features(config, args))
                    ...
                    with maybe_no_sync():
                        ...
                        for name, p in self.model.named_parameters():                 
                            ...
                            if p.grad is not None:
                                if ci == len(configs)-1:
                                    largest_net_grads[num_params:num_params+nn] += p.grad.view(-1).cpu()
                                else:
                                    subnet_grads[num_params:num_params+nn] += p.grad.view(-1).cpu()
                                num_done += nn
                            num_params += nn
                        self.zero_grad()
        ...
        subnet_grads /= len(configs)-1
        print("Grad-Conflict-Cosine-Sim: %.2f"%(distance.cosine(subnet_grads.numpy(), largest_net_grads.numpy())))
        return None
    ...
    def valid_step(self, sample, generator=None, raise_oom=False):
        """Do forward pass in evaluation mode."""
        if generator:
            gold_target, pred_target = [], []
        with torch.no_grad():
            self.model.eval()
            self.criterion.eval()
            ...
            try:
                ...
                if generator:
                    ...
                    hypos, decoder_times = self.task.inference_step(generator, [self.model], sample, None)
                    for i, sample_id in enumerate(sample['id'].tolist()):
                        src_tokens = utils.strip_pad(sample['net_input']['src_tokens'][i, :], tgt_dict.pad())
                        src_str = src_dict.string(src_tokens, None)
                        target_tokens = utils.strip_pad(sample['target'][i, :], tgt_dict.pad()).int().cpu()
                        target_str = tgt_dict.string(target_tokens, None, escape_unk=True)
                        gold_target.append(target_str)
        ...
        if generator:
            return logging_output, (gold_target, pred_target)
