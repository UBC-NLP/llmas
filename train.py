...
def main(args, init_distributed=False):
    ...
    if args.profile_flops:
        ...
        model.set_sample_config(config_subtransformer, arch_embeds=utils.get_config_features(config_subtransformer, None))
    ...
    if args.validate_subtransformer:
        ...
        trainer.set_sample_config(config, arch_embeds=utils.get_config_features(config, args))
    ...
    while lr > args.min_lr and epoch_itr.epoch < max_epoch and trainer.get_num_updates() < max_update:
        ...
        if not args.disable_validation and epoch_itr.epoch % args.validate_interval == 0:
            for k, v in represent_configs.items():
                trainer.set_sample_config(config=v, arch_embeds=utils.get_config_features(v, args))
...
def train(args, trainer, task, epoch_itr, generator=None):
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        if args.train_subtransformer:
            ...
            for i in range(4):
                configs += [utils.sample_configs(utils.get_all_choices(args), reset_rand_seed=True, rand_seed=trainer.get_num_updates(), super_decoder_num_layer=args.decoder_layers)]
            if args.sandwich_rule == 1:
                # configs += [represent_configs["largest_arbitrary1"], represent_configs["smallest_arbitrary1"]]
                configs += [represent_configs["largest_arbitrary1"]]
            new_config1 = copy.deepcopy(represent_configs["smallest_arbitrary1"])
            new_config1["encoder"]["encoder_embed_dim"] = 640
            new_config1["decoder"]["decoder_embed_dim"] = 640
            new_config2 = copy.deepcopy(represent_configs["largest_arbitrary1"])
            new_config2["encoder"]["encoder_embed_dim"] = 512
            new_config2["decoder"]["decoder_embed_dim"] = 512
            configs = [represent_configs["smallest_arbitrary1"], represent_configs["largest_arbitrary1"]]
        log_output = trainer.train_step(samples, configs=configs, args=args)
        ncount += 1
        if ncount > 55:
            sys.exit(0)
